...
//    nama tabel dan primary key
    private $table = 't_inbound';
    private $pk = 'id_inbound';
	...
	public function getlist_paging($limit, $start){
		...
		$q = $this->db->get('t_inbound a',$limit,$start); 
        return $q;
    }

    public function getById($id_inbound) {
        $q = $this->db->where($this->pk,$id_inbound);
    ...
    public function ubah($id_inbound,$data) {

        $this->db->where($this->pk, $id_inbound);
    ...
    public function hapus($id_inbound) {
        $this->db->where($this->pk, $id_inbound);
	...
	public function get_product_keyword($keyword,$limit,$start){
		$this->db->select('a.*,b.nama_barang');
		$this->db->join('m_item b', 'a.id_barang = b.id_barang', 'left');
		$this->db->like('a.tgl_inbound',$keyword);
	...
	public function getlist_print($datefrom,$dateto,$idbarang){
		...
		if ($datefrom != null){
			$this->db->where('a.tgl_inbound >=', $datefrom);
		}
		if ($dateto != null){
			$this->db->where('a.tgl_inbound <=', $dateto);
		}
		$q = $this->db->order_by('a.tgl_inbound asc');
		$q = $this->db->get('t_inbound a'); 
        return $q;
    }
}
